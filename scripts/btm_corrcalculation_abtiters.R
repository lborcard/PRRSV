btm_corrcalculation_abtiters <- function(data_for_corr= superdf_abtiter,
                                Daystocompare=c(10,7),
                                btm_family="b",
                                myvalue="titers_log2",
  ...
  myfavcorr<-data_for_corr %>%
    filter(dpi==Daystocompare[1])%>%#we set the day we want to use for the titers data
    ...
    pivot_wider(names_from = genesets,values_from = es,values_fn = mean)
  ...
  namesettings<-paste(treatmentofinterest,sep = "_")
  ...
  plotrescor<-res.cor_btm %>%
    ...
    filter(Family_name=="ag presentation")%>% #"ag presentation"
