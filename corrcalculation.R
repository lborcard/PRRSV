corrcalculation <- function(data_for_corr= superdf_clean,Daystocompare=c(0,0),
  ...
  plotrescor<-res.cor %<>%
    ...
    geom_tile()+
    ggtitle(paste("Day",Daystocompare[1],(celltype)," vs ","Day",Daystocompare[2],"(ES)",treatmentofinterest,sep = ""),
            subtitle = paste("cutoff p<0.005"," correlation value >abs(",corr_value,")",sep = ""))+
    #coord_fixed(ratio = 1)+
    coord_equal()+
    coord_flip() +
    theme_classic()
